...
#include "lf_queue_v2.h"
#include "mem_pool_v2.h"
#include "thread_utils_v2.h"
#include "macros.h"
...
void benchmarkSPSCQueue() {
  ...
  std::cout << "Throughput: " << std::fixed << std::setprecision(2) 
            << (NUM_ITEMS * 1000000.0 / duration) << " items/sec" << std::endl;
  std::cout << "Latency per item: " << (duration * 1000.0 / NUM_ITEMS) << " ns" << std::endl;
}

// Benchmark Memory Pool
void benchmarkMemPool() {
  ...
  // Single-threaded pool
  {
    ...
    std::cout << "Average allocation time: " << (duration / NUM_ALLOCS) << " ns" << std::endl;
  }
