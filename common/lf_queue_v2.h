...
namespace Common {
  ...
  // Helper to align data to cache line boundaries
  template<typename T>
  ...
  public:
    ...
    auto getNextToWriteTo() noexcept -> T* {
      const auto write_idx = write_index_.data.load(std::memory_order_relaxed);
      const auto next_write = (write_idx + 1) & (capacity_ - 1);
      
      // Check if queue is full
      if (UNLIKELY(next_write == read_index_cache_)) {
        read_index_cache_ = read_index_.data.load(std::memory_order_acquire);
    ...
    auto updateWriteIndex() noexcept {
      const auto write_idx = write_index_.data.load(std::memory_order_relaxed);
      write_index_.data.store((write_idx + 1) & (capacity_ - 1), 
                              std::memory_order_release);
    }
    
    // Consumer side - only called by consumer thread
    auto getNextToRead() noexcept -> const T* {
      const auto read_idx = read_index_.data.load(std::memory_order_relaxed);
      
      // Check if queue is empty
      if (UNLIKELY(read_idx == write_index_cache_)) {
        write_index_cache_ = write_index_.data.load(std::memory_order_acquire);
    ...
    auto updateReadIndex() noexcept {
      const auto read_idx = read_index_.data.load(std::memory_order_relaxed);
      read_index_.data.store((read_idx + 1) & (capacity_ - 1), 
                             std::memory_order_release);
    }
    
    // Approximate size - not exact due to concurrent access
    auto size() const noexcept {
      const auto write = write_index_.data.load(std::memory_order_acquire);
      const auto read = read_index_.data.load(std::memory_order_acquire);
  ...
  public:
    ...
    bool enqueue(const T& item) noexcept {
      std::size_t pos = write_index_.data.load(std::memory_order_relaxed);
      ...
      for (;;) {
        ...
        if (diff == 0) {
          ...
          if (write_index_.data.compare_exchange_weak(pos, pos + 1, 
          ...
          pos = write_index_.data.load(std::memory_order_relaxed);
        }
      }
    }
    ...
    bool dequeue(T& item) noexcept {
      std::size_t pos = read_index_.data.load(std::memory_order_relaxed);
      ...
      for (;;) {
        ...
        if (diff == 0) {
          // Try to claim this slot
          if (read_index_.data.compare_exchange_weak(pos, pos + 1, 
                                                     std::memory_order_relaxed)) {
            item = cell.data;
          ...
          // Another thread got this slot, try next
          pos = read_index_.data.load(std::memory_order_relaxed);
        }
      }
    }
